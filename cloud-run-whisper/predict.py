...
class WhisperDiarizer:
    ...
    def setup(self) -> None:
        """Load all models into GPU memory. Call once at server startup."""
        logger.info("Loading Whisper model from /models/faster-whisper-large-v3-turbo")
    ...
    def _speech_to_text(
        ...
        final_segments = self._merge_segments(segments, diarize_df, speaker="UNKNOWN")

        # --- Group same-speaker segments ---
        if len(final_segments) > 0 and group_segments_gap > 0:
            final_segments = self._group_segments(final_segments, group_segments_gap)
